...
const navMenu = document.querySelectorAll<HTMLElement>('nav li');
...
const sliderHtml = () => {
    let insertHtml = '';
    mainSliderContents.forEach(
        (sliderElem) =>
            (insertHtml += `<article class='${sliderElem.styleName}'>
...
const mainSliderContainer = document.querySelector<HTMLElement>('.wrapper');
mainSliderContainer?.insertAdjacentHTML('afterbegin', sliderHtml());

const controlSlider = document.querySelectorAll<HTMLElement>('.wrapper__img');
const sliderBtn = document.querySelectorAll<HTMLElement>('.btn');
...
// 마우스오버시 동작하는 이벤트 대상
const imgBox = document.querySelectorAll<HTMLElement>(
    '.briefintroduction__container__imgbox__img'
);

// 마우스오버시 스타일이 변경될 대상
const textBox = document.querySelectorAll<HTMLElement>(
    '.briefintroduction__container__textbox'
);
